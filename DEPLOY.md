...
YNAB account the server reaches. The Worker uses one server-wide YNAB token, so
anyone admitted through GitHub reaches the deployer's YNAB account, money, and
every budget available to that token—not their own YNAB account.
